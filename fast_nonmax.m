...
function ret = fast_nonmax(im, barrier, c, maxPoints)
	sz = size(im);
	ysize = sz(1);
    ...
    % Sort center positions according to their pixel scores
	[~, centres] = findpeaks(scores, 'SORTSTR', 'descend');
	...
	maximas = centres(find(cs, maxPoints, 'first'));
...
%     scoresIm = reshape(scores, size(im, 1), size(im, 2));
%     contourf(scoresIm); hold on;
%     plot(ret(:, 1), ret(:, 2), 'oy');
%     hold off
